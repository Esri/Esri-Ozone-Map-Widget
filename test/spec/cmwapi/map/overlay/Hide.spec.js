define(["cmwapi/Channels", "cmwapi/map/overlay/Hide", "cmwapi/map/Error", "cmwapi/Validator",
    "test/mock/OWF", "test/mock/Ozone"], 
    function(Channels, Hide, Error, Validator, OWF, Ozone) {

    describe(Channels.MAP_OVERLAY_HIDE + " module", function() {
        ...
        it("sends data to the correct channel", function() {
            ...
            spyOn(Hide, 'send').andCallThrough();
            spyOn(eventing, 'publish');
            spyOn(Error, 'send');

            Hide.send();
            expect(Hide.send).toHaveBeenCalled();

            // expect publish to be called
            expect(eventing.publish).toHaveBeenCalled();
            expect(eventing.publish.mostRecentCall.args[0]).toEqual(Channels.MAP_OVERLAY_HIDE);
        ...
        it("unsubscribes the correct channel when removeHandlers is called", function() {
            ...
            spyOn(Hide, 'removeHandlers').andCallThrough();
            spyOn(Error, 'send');
            spyOn(eventing, 'unsubscribe');

            Hide.removeHandlers();
            expect(Hide.removeHandlers).toHaveBeenCalled();
            expect(eventing.unsubscribe.mostRecentCall.args[0]).toEqual(Channels.MAP_OVERLAY_HIDE);
        ...
        it("wraps added handlers and fills in missing overlay id", function() {
            ...
            var newHandler = Hide.addHandler(testHandler);
            expect(eventing.subscribe.mostRecentCall.args[0]).toEqual(Channels.MAP_OVERLAY_HIDE);

            // Test the behavior for newHandler  Hide a sender an empty payload to pass along
