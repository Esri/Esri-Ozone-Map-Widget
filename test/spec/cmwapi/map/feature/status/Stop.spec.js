 ...
 */
define(["cmwapi/Channels", "cmwapi/map/feature/status/Stop", "cmwapi/map/Error", "cmwapi/Validator",
    "test/mock/OWF", "test/mock/Ozone"],
    function(Channels, Stop, Error, Validator, OWF, Ozone) {

    describe("Verify " + Channels.MAP_FEATURE_STATUS_STOP + " module", function() {
        ...
        it("sends data to the correct channel", function() {
            ...
            spyOn(Stop, 'send').andCallThrough();
            spyOn(eventing, 'publish');
            spyOn(Error, 'send');

            Stop.send({eventType: 'mouse-over', overlayId: 'o', featureId: 'f', subfeatureId: 'sf'});
            expect(Stop.send).toHaveBeenCalled();
        ...
        it("errors on bad event type", function() {
            ...
            Stop.send({eventType: 'fake-event'});
            expect(Stop.send).toHaveBeenCalled();
        ...
        it("errors on bad data object", function() {
            ...
            //pre json encoded
            Stop.send( Ozone.util.toString({eventType: 'fake-event'}));
            expect(Stop.send).toHaveBeenCalled();
        ...
        it("unsubscribes the correct channel when removeHandlers is called", function() {
            ...
            spyOn(Stop, 'removeHandlers').andCallThrough();
            spyOn(Error, 'send');
            spyOn(eventing, 'unsubscribe');

            Stop.removeHandlers();
            expect(Stop.removeHandlers).toHaveBeenCalled();
        ...
        it("wraps added handlers and passes along any optional elements", function() {
            ...
            var newHandler = Stop.addHandler(testHandler);
